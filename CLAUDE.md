...
- ✅ Per-directory profile: `~/.config/cc-shisa/locations.json` maps `realpath` → `{level?, overrides?}`. CLI: `cc-shisa here [set-level|set-override|clear-override|unset]` (cwd-only) and `cc-shisa locations [list|unset]`. Longest-path-prefix match. Only `level` + `overrides` are per-directory; modules continue to come from the global profile
